...
def first_odd_or_even(numbers):
    ...
    if even == odd or even == 0 or odd == 0:
        return 0
    return next(x for x in numbers if x % 2 == 1) if even > odd else next(x for x in numbers if x % 2 == 0)
...
def to_pilot_alpha(word):
    ...
    for char in list(word):
        pilot_alpha_list.append(
            next(x for x in pilot_alpha if char.upper() == x[0]))
