...
def gen_quiz(qpool, *inidicies, altcodes='ABCDEF', quiz=None):
    """
    ...
    for i in inidicies:
        try:
            buildAnswer = []
            for answer in zip(altcodes, qpool[i][1]):          
                buildAnswer.append(answer[0]+": "+answer[1])  
            if quiz is None:
                quiz=[]  
            quiz.append((qpool[i][0], buildAnswer))
